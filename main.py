...
from agents.router import extract_job_parameters_and_intent, generate_resume_draft, JobRequest
...
def handler_match_jobs(request: JobRequest):
    ...
    roles = request.roles
    if not roles:
        role_fallback = input("No job roles were detected. Please enter a job role: ").strip()
        if role_fallback:
            roles = [r.strip() for r in role_fallback.split(",") if r.strip()]
        else:
    ...
    if not skills:
        ...
        skills_fallback = input("Enter your skills (comma-separated), or press Enter to skip: ").strip()
        if skills_fallback:
            skills = [s.strip() for s in skills_fallback.split(",") if s.strip()]
...
def handler_list_jobs(request: JobRequest):
    """
    Handler responsible for searching and listing jobs via Parallel Tool Calling without full match evaluation.
    """
    print("\n--- [Handler: List Jobs] ---")
    ...
    for number, job in enumerate(all_jobs, start=1):
        ...
        print(f"   Location: {job['location']}")
        print(f"   URL: {job['url']}")
        desc_snippet = job.get('description', '').replace('\n', ' ')[:150]
        if desc_snippet:
...
def handler_resume_creator(request: JobRequest):
    """
    Handler responsible for creating or tailoring a resume based on candidate
    skills and target roles.
    """
    print("\n--- [Handler: Resume Creator] ---")
    print("Generating tailored resume recommendations using LangChain...\n")
...
def main():
    ...
    user_prompt = input(
        "\nEnter your request (e.g. 'Match Python Developer jobs in Texas for Python, SQL'): "
    ).strip()
    ...
    print("\nProcessing request and extracting parameters with LangChain...")
    job_request = extract_job_parameters_and_intent(user_prompt)

    print("\n" + "=" * 60)
    print("REQUEST EXTRACTION & ROUTING")
    print("=" * 60)
    print(f"Detected Intent : {job_request.intent}")
    print(f"Target Roles    : {', '.join(job_request.roles) if job_request.roles else 'None detected'}")
    print(f"Target Location : {', '.join(job_request.location) if job_request.location else 'United States'}")
    print(f"Candidate Skills: {', '.join(job_request.skills) if job_request.skills else 'None detected'}")
    print("=" * 60)

    handler = HANDLERS.get(job_request.intent, handler_match_jobs)
    handler(job_request)
